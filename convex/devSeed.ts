 ...
 *
 * ⚠️ SECURITY: All endpoints require authentication.
 *    seedGarageData en fixTenantTokens vereisen eigenaar-rol.
 *    debugTokens vereist eigenaar-rol.
 */
...
/**
 * seedGarageData — Zaait demo-data voor de garage.
 *
 * KR-2 FIX: Vereist eigenaar-rol. Gebruikt profiel.tokenIdentifier als
 * correcte tenant-anchor in plaats van de raw JWT tokenIdentifier.
 * Dit voorkomt dat een niet-eigenaar data aan de verkeerde tenant koppelt.
 */
export const seedGarageData = mutation({
    args: {},
    handler: async (ctx) => {
        // KR-2 FIX: eigenaar-only + juiste tenant-anchor ophalen
        const profiel = await requireDomainRole(ctx, "eigenaar");
        const ti = profiel.tokenIdentifier;
        const now = Date.now();
        ...
        // ─────────────────────────────────────────────────────────────────────
        // VOERTUIGEN — genormaliseerde kentekens (uppercase, geen streepjes)
        // ─────────────────────────────────────────────────────────────────────
        const voertuigData = [
            {
                klantEmail: "jan.devries@gmail.com",
                kenteken: "GH446V", merk: "Peugeot", model: "207",
            ...
            {
                klantEmail: "jan.devries@gmail.com",
                kenteken: "SB756T", merk: "Volkswagen", model: "Golf VII",
            ...
            },
            {
                ...
                kenteken: "LT348R", merk: "Toyota", model: "Yaris",
            ...
            },
            {
                ...
                kenteken: "VN123B", merk: "Mercedes-Benz", model: "Sprinter 315 CDI",
            ...
            {
                klantEmail: "info@prinstransport.nl",
                kenteken: "KZ456X", merk: "Ford", model: "Transit Custom",
            ...
            },
            {
                ...
                kenteken: "RT789N", merk: "Renault", model: "Clio V",
            ...
            },
            {
                ...
                kenteken: "HF234G", merk: "BMW", model: "3 Serie",
            ...
            },
            {
                ...
                kenteken: "BK567P", merk: "Citroën", model: "Berlingo",
            ...
            {
                klantEmail: "lisa@vandenbergcatering.nl",
                kenteken: "ZN890W", merk: "Opel", model: "Vivaro",
            ...
            {
                klantEmail: "sandra.bakker@hotmail.nl",
                kenteken: "DA112M", merk: "Kia", model: "Niro EV",
        ...
        for (const v of voertuigData) {
            ...
            const id = await ctx.db.insert("voertuigen", {
                klantId,
                kenteken: v.kenteken, // Al genormaliseerd in de data
                merk: v.merk,
                model: v.model,
                bouwjaar: v.bouwjaar,
...
// ── 3. Debug: inspect tokens (DEV ONLY — eigenaar-only) ────────────────────

/**
 * debugTokens — inzicht in token-staat voor debugging.
 *
 * KR-1 FIX: vereist eigenaar-rol. Voorheen had dit endpoint geen auth-check,
 * waardoor iedereen alle tokenIdentifiers en userIds kon opvragen.
 */
export const debugTokens = query({
    args: {},
    handler: async (ctx) => {
        // KR-1 FIX: eigenaar-only — gevoelige token-informatie
        const profiel = await requireDomainRole(ctx, "eigenaar");
 ...
 *
 * KR-1 FIX: vereist eigenaar-rol. Voorheen had dit endpoint geen auth-check,
 * waardoor iedereen tenant-records kon muteren of verwijderen.
 *
...
export const fixTenantTokens = mutation({
    args: {},
    handler: async (ctx) => {
        // KR-1 FIX: eigenaar-only — muteert medewerker-records
        const identity = await ctx.auth.getUserIdentity();
